...
def fit_trendline(year_timesteps, data):
    ...
    try:
        ...
        logging.error(f"Both lists must contain floats or integers, got {year_timesteps.dtype} and {data.dtype} instead.") 
        logging.exception(e)
        return 0.0, 0.0
    else:
        # if no error occurs
        ...
        logging.info(f"Completed fit_trendline function, Slope is {slope}, r_squared is {r_squared}")
